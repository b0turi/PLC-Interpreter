...
(define M_state_assign
  (lambda (class varname value s goto api instance)
...
(define M_state_block
  (lambda (class stmt-lis s goto api instance)
    ; Each block of code exists within its own layer on top of the state, which must be removed after the block is completed
    (remove_layer (M_state_list class stmt-lis (add_layer s) (block_goto goto) api instance))))
...
(define M_state_declare
  (lambda (varname value s api instance)
    (cond
...
(define M_state_function
  (lambda (class fname args s goto api instance)
    (call/cc
     (lambda (return)
       (cond
         ((not (exist? fname s)) (error "Function does not exist"))
         (else (begin (M_state_list class (fclosure-body (lookup fname s)) (remove-layer fname (fsetup (fclosure-params (lookup fname s)) args (add_layer s) goto)) (goto-setup 'return (gotoreturn return s) (func-goto s goto)) api instance) s)))))))
...
(define M_state_if
  (lambda (class condition then-statement else-statement s goto api instance)
    (if (M_boolean class condition s goto api instance)
        (M_state class then-statement s goto api instance)
        (M_state class else-statement s goto api instance))))
...
(define M_state_list
  (lambda (class stmt-lis s goto api instance)
    (cond
      ((null? stmt-lis) s)
      (else (M_state_list class (next stmt-lis) (M_state class (first stmt-lis) s goto) goto api instance)))))
...
(define M_state_main
  (lambda (class s goto api instance)
    (M_value_function class 'main '() s goto api instance)))
    
; M_state
; Given a statement and a state, evaluate the statement and return the new state
(define M_state
  (lambda (class stmt s goto api instance)
    (cond
      ...
      ((eq? (operator stmt) 'var) (M_state_declare (var-name stmt) (M_value (assignment stmt) s goto api instance) (M_state_side-effect (assignment stmt) s goto api instance)))
      ...
      ((eq? (operator stmt) 'return) (goto 'return (realvalue (M_value class (return-exp stmt) (M_state_side-effect (assignment stmt) s goto api instance) goto api instance))))
      ((eq? (operator stmt) 'throw) (goto 'throw (throws (realvalue (M_value class (return-exp stmt) s goto api instance)) s)))
      ((state-goto? (operator stmt)) (goto (operator stmt) s))
      ((eq? (operator stmt) 'try) (M_state_try class stmt s goto api instance))
      
      ; Check if the statement is a branching statement with a condition
      ((eq? (operator stmt) 'while) (M_state_while-start class (condition stmt) (body stmt) s goto api instance))
      ((eq? (operator stmt) 'if) (M_state_if class (condition stmt) (then stmt) (else stmt) s goto api instance))

      ; Check if the statement is a block of code, defined by "begin" in the parser
      ((eq? (operator stmt) 'begin) (M_state_block class (block-body stmt) s goto api instance))
...
(define M_state_side-effect
  (lambda (class stmt s goto api instance)
    (cond
      ...
      ((eq? (operator stmt) '=) (M_state_assign class (var-name stmt) (M_value class (assignment stmt) s goto api instance) (M_state_side-effect class (assignment stmt) s goto api instance) goto api instance))
  
      ; Check if the statement is another kind of expression
      ((single_value? stmt) (M_state_side-effect class (operand1 stmt) s goto api instance))
      ((dual_value? (operator stmt)) (M_state_side-effect class (operand2 stmt) (M_state_side-effect class (operand1 stmt) s goto api instance) goto api instance))
      (else s))))

; M_state_class_function_closures
(define M_state_class_function_closures
  (lambda (class body s goto api instance)
    (cond
      ((null? body) s)
      ((or (eq? (operator (first body)) 'function) (eq? (operator (first body)) 'static-function)) (M_state_class_function_closures class (next body) (M_state class (first body) s goto api instance) goto api instance))
      (else (M_state_class_function_closures class (next body) s goto api instance)))))

; M_state_while-start
; A helper function for M_state_while using the current continuation to keep track of the state
(define M_state_while-start
  (lambda (class condition body-statement s goto api instance)
    (call/cc
     (lambda (break)
       (M_state_while class condition body-statement s (goto-setup 'break break goto) api instance)))))
...
(define M_state_while
  (lambda (class condition body-statement s goto api instance)
    (if (M_boolean class condition s goto api instance)
        (M_state_while class condition body-statement (M_state-continue class body-statement s goto api instance) goto api instance)
        s)))
...
(define M_state-continue
  (lambda (class stmt s goto api instance)
    (call/cc
     (lambda (continue)
       (M_state class stmt s (goto-setup 'continue continue goto) api instance)))))
...
(define M_state_try
  (lambda (class stmt s goto api instance)
    (cond
      ((and (catch? stmt) (finally? stmt)) (M_state_block class (finally-body stmt) (M_state_try_with-catch class stmt s goto api instance) goto api instance))
      ((catch? stmt) (M_state_try_with-catch class stmt s goto api instance))
      ((finally? stmt) (M_state_block class (finally-body stmt) (M_state_try_without-catch class (try-body stmt) s goto api instance) goto api instance))
      (else (M_state_try_without-catch class (try-body stmt) s goto api instance)))))
...
(define M_state_try_with-catch
  (lambda (class stmt s goto api instance)
    (call/cc
     (lambda (throw)
       (M_state_block class (try-body stmt) s (goto-setup 'throw (lambda (t) (throw (M_state_catch stmt class (throw-value t) (remove_layer (throw-state t)) goto))) goto) api instance)))))
...
(define M_state_catch
  (lambda (class stmt e s goto api instance)
    (M_state_block class (catch-body stmt) (M_state_declare class (catch-var stmt) e s) goto api instance)))


; M_boolean
; Given a boolean statement and a state, return the boolean value of the statement
(define M_boolean
  (lambda (class b-stmt s goto api instance)
    (cond
      ((exp? b-stmt) (M_evaluate class b-stmt s goto api instance))
...
(define M_value
  (lambda (class stmt s goto api instance)
    (cond
      ((exp? stmt) (M_evaluate class stmt s goto api instance))
  ...
  (lambda (class fname args s goto api instance)
    (call/cc
     (lambda (return)
       (cond
         ((not (exist? fname s)) (error "Function does not exist"))
         (else (M_state_list class (fclosure-body (lookup fname s)) (remove-layer fname (fsetup (fclosure-params (lookup fname s)) args (add_layer s) goto)) (goto-setup 'return return (func-goto s goto)) api instance)))))))
...
(define M_evaluate
  (lambda (class exp s goto api instance)
    (cond
      ...
      ((unary-? exp) (- 0 (M_value class (operand1 exp) s goto api instance)))
      ((eq? (operator exp) '!) (not (M_boolean class (operand1 exp) s goto api instance)))
      ((eq? (operator exp) '=) (M_value class (assignment exp) (M_state_side-effect class (assignment exp) s goto api instance) goto api instance))
      ((value_op? (operator exp)) (operation (operator exp) (M_value class (operand1 exp) s goto api instance) (M_value class (operand2 exp) (M_state_side-effect class (operand1 exp) s goto api instance) goto api instance)))
      ((bool_op? (operator exp)) (operation (operator exp) (M_boolean class (operand1 exp) s goto api instance) (M_boolean class (operand2 exp) (M_state_side-effect class (operand1 exp) s goto api instance) goto api instance)))
